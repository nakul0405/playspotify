...
async def login(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "Tap below to log in and automatically send your Spotify cookie 👇\n(𝘾𝙤𝙥𝙮 𝙩𝙝𝙚 𝙡𝙞𝙣𝙠 𝙖𝙣𝙙 𝙤𝙥𝙚𝙣 𝙞𝙣 𝘾𝙝𝙧𝙤𝙢𝙚/Safari 𝙞𝙛 𝙏𝙚𝙡𝙚𝙜𝙧𝙖𝙢 𝙗𝙧𝙤𝙬𝙨𝙚𝙧 𝙙𝙤𝙚𝙨𝙣’𝙩 𝙬𝙤𝙧𝙠)",
...
async def mytrack(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if current:
            ...
            msg = "😴 No current track found. Maybe you’re not listening to anything?"
        await update.message.reply_text(msg, parse_mode="Markdown")
    except Exception as e:
        await update.message.reply_text(f"❌ Error: {e}")
...
async def logout(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id in cookies:
        ...
        with open(cookies_file, "w") as f:
            json.dump(cookies, f, indent=2)
        await update.message.reply_text("🚪 You’ve been logged out. Spotify tracking disabled.")
    else:
        await update.message.reply_text("⚠️ You're not logged in. Use /login or /setcookie first.")
...
async def hello(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    reply_markup = InlineKeyboardMarkup(keyboard)
    await update.message.reply_text(
        "✅ Hey there! I’m *PlaySpotify* — your music spy bot 🕵️‍♂️🎧\n\n"
        "Click below to try me or talk to my creator 👇",
        reply_markup=reply_markup,
        parse_mode="Markdown"
    )
